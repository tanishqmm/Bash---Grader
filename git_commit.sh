...
if [ ${#New_files[@]} -eq 0 ] && [ ${#Modified_files[@]} -eq 0 ] && [ ${#deletedfiles[@]} -eq 0 ];then
    echo "No changes have been made in csv files so commit not possible"
else
    ...
    ##Writing details of hash value message and date in git_log
    echo "commit: ${Hash_value} |||| message: \"$1\"" >>$path/$currentbranch/.git_log
    echo `date ` >>$path/$currentbranch/.git_log
    echo >>$path/$currentbranch/.git_log

    ##Displaying same on terminal
    echo "commit: ${Hash_value} |||| message: \"$1\"" 
    echo `date`
    echo 

    ##creating directory of that hash value where we will store all the csv files 
    mkdir $path/$currentbranch/$Hash_value 
    cp *.csv $path/$currentbranch/$Hash_value
